...
## Step 2 — set env vars บน enabridge-site (Vercel)

เปิด Vercel dashboard → project `enabridge-site` → Settings → Environment Variables:

| Key | Value | Scope |
|---|---|---|
| `GITHUB_OWNER` | `<YOUR-USER>` (เช่น `enabridge` หรือ `yoh-enabridge`) | Production + Preview |
| `GITHUB_REPO` | `enabridge-research` | Production + Preview |
| `GITHUB_BRANCH` | `main` | Production + Preview |

Redeploy (push empty commit หรือกด "Redeploy" ใน Vercel)
...
| Feed ว่าง (`<channel>` ไม่มี `<item>`) | `curl https://raw.githubusercontent.com/<YOUR-USER>/enabridge-research/main/audio/index.json` ได้ไหม? ถ้า 404 = ยังไม่ได้ push, ถ้า got JSON = ตรวจ env vars บน Vercel |
| RSS feed cache ค้าง | Wait 5 min (revalidate=300) หรือ force redeploy Vercel |
...
2. ☐ set `GITHUB_OWNER` / `GITHUB_REPO` บน Vercel + redeploy (Step 2)
